...
def log_likelihood(probability, indices, gold_indices, gold_labels, invalid_rate = 1.0):
    """
    ...
    for batch_idx, label in enumerate(gold_indices):
        for i, l in enumerate(label):
            if l in indices[batch_idx]:
                ...
                invalid_rate = invalid_rate if gold_labels[batch_idx][i] == 0 else 1.0
                gold_indice_labels.append((batch_idx, idx, gold_labels[batch_idx][i], invalid_rate))

    # sum of the negative log-likelihood from both the mention module and triplet module
    loss = [-torch.log(probability[c[0], c[1], c[2]]) * c[3] for c in gold_indice_labels]
